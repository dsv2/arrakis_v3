...
select BOOK.ID AS BOOK_ID, SECURITY.ID AS SECURITY_ID, COUNTERPARTY.ID AS COUNTERPARTY_ID, SECURITY.CURRENCY AS CURRENCY, SECURITY.STATUS AS STATUS,FULL_DATA.QUANTITY AS
QUANTITY, FULL_DATA.UNIT_PRICE AS UNIT_PRICE,FULL_DATA.TRADE_TYPE AS BUY_SELL,PARSEDATETIME(FULL_DATA.TRADE_DATE, 'dd/MM/yyyy') AS TRADE_DATE, PARSEDATETIME(FULL_DATA.BOND_MATURITY_DATE, 'dd/MM/yyyy')
AS SETTLEMENT_DATE FROM SECURITY
